...
// Formulário para criar/editar ONG
...
const AdminOngsScreen = () => {
	...
	useEffect(() => {
		if (isFocused) { loadOngs(); }
	}, [isFocused]);

	const handleOpenModal = (ong = null) => {
		setSelectedOng(ong);
	...
	const handleSaveOng = async (ongData) => {
		...
		try {
			if (selectedOng) { 
				await updateOng(selectedOng.id, ongData);
			} else { 
			...
			Alert.alert('Erro', 'Não foi possível salvar a ONG.');
		} finally {
			setIsLoading(false);
		}
	};

	const handleDeleteOng = (ong) => {
		Alert.alert('Confirmar Deleção', `Tem certeza que deseja deletar a ONG "${ong.name}"?`,
			[
				{ text: 'Cancelar' },
				{
					text: 'Deletar', style: 'destructive', onPress: async () => {
						try {
							await deleteOng(ong.id);
							await loadOngs();
						} catch (error) {
							Alert.alert('Erro', 'Não foi possível deletar a ONG.');
						}
					}
				}
			]
	...
	return (
		<View style={styles.container}>
			...
			<FlatList
				...
				renderItem={({ item }) => (
					<OngItem item={item} onEdit={handleOpenModal} onDelete={handleDeleteOng} />
				)}
				onRefresh={loadOngs}
